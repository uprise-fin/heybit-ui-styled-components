...
import { Color, colors, colorPalette, prefix, basicVariables, componentVariables } from './type';
...
export default (function setRootStyleProperty() {
  if (!document.documentElement.style.getPropertyValue(`--${prefix}__${colorPalette.black}`)) {
    // pallete
    Object.entries(colorPalette).forEach(([origin, obj]: [Color, Matercolor]) => {
      setProperty(`--${prefix}__${origin}`, colors[origin]);
      Object.entries(obj).forEach(([level, color]) => {
        if (!(+level > 0)) return;
        if (colors[origin] === color) setProperty(`--${prefix}__${origin}--primary`, level);
